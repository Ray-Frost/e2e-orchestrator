...
1. Add the backend statistics endpoint and minimal Prisma plumbing in
   `apps/server/src/**`.
   Validation: Nest HTTP tests cover empty, grouped, tie-broken, and
   `last_failed_at DESC` / `last_run_id DESC` responses for
...
3. Wire the frontend dev proxy in `apps/web/vite.config.ts` and keep API calls
   relative.
   Validation: dev and build-time configuration resolve `/api/...` requests to
   the server port without adding backend CORS.
